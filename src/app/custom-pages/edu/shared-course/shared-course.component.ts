...
export class SharedCourseComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      ...
      } else if (shareStatus === 'pgd') {
        this.trangThaiChiaSe = 1;
        this.subText = 'PGD&ĐT';
      }
      else if (shareStatus === 'gvg') {
        this.trangThaiChiaSe = 2;
        this.subText = 'dự thi giáo viên giỏi';
      }
      forkJoin([
        ...
        this.filterGradeSource = [1, 2, 3, 4, 5, 6, 7, 8, 9];
  ...
  async loadGrid() {
    ...
    } else if (this.trangThaiChiaSe == 2) {
      {
        this.generalService.getKhoaHocDuThiGiaoVienGioi(payload).subscribe(res => {
          this.isLoading = false;
          this.datas = res.items;
          this.totalItem = res.totalCount;
          this.schoolCourseCount = ` (${res.totalCount} khóa học)`;
