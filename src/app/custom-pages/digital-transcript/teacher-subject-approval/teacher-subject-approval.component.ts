...
@Component({
  selector: 'app-subject-teacher-homeroom-approval',
  templateUrl: './teacher-subject-approval.component.html',
  styleUrls: ['./teacher-subject-approval.component.scss']
})
export class TeacherSubjectApprovalComponent implements OnInit {
  isShowEdit = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
  ...
  applyFilter(teacherGD){
    ...
    this.teacherAssignSource = teacherGD.phaN_CONG_GIANG_DAY;
  ...
  async loadGrid() {
    this.generalService.getLopGDBySubjectTeacher(this.schoolId, this.teacherGD.id, this.filterSchoolYear, this.filterSubjectId, this.filterClassIds).subscribe(res => {
      this.datas = res;
      let index = 1;
  ...
  showDetail(data: any) {
    ...
    if (this.isElementary) {
      ...
      this.generalService.getDiemGVBMChiTietC2New(this.schoolId, this.user.data.personId, this.filterSchoolYear, this.filterSubjectId, data.classId).subscribe(res => {
        this.studentSource = res.lst;
    ...
    result.then((dialogResult) => {
      if (dialogResult) {
        if (this.staffInfo && this.staffInfo.signatureProvider === 0) {
          this.signWithVietelMySign();
        } else {
          this.signWithTokenBCY();
        }
      }
    });
  }

  signWithVietelMySign() {
    this.startCountdown();
    this.isShowSignLoading = true;
    let payload = {
    ...
    this.generalService.signMultipleByGVBM(payload).subscribe(en => {
      this.isShowSignLoading = false;
      if (en.code === 0) {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xác thực ký thành công');
        this.loadGrid();
