...
@Component({
  selector: 'app-transcript-list-c1',
  templateUrl: './transcript-list-c1.component.html',
  styleUrls: ['./transcript-list-c1.component.scss']
})
export class TranscriptListC1Component implements OnInit {
  isShowEdit = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
  ...
  async ngOnInit() {
    ...
    forkJoin([
      ...
      if (this.filterClassSource.length) {
  ...
  doSign() {
    ...
    result.then((dialogResult) => {
      if (dialogResult) {
        ...
        this.generalService.signByGVCNVer2(this.dataItem.id, this.isElementary).subscribe(en => {
          if (en.code === 0) {
  ...
  handleSelectionChange($event) {
    ...
    this.status0Count = $event.selectedRowKeys.filter(en => en.status == 0).length;
    ...
    this.status50Count = $event.selectedRowKeys.filter(en => en.status == 50).length;
          ...
          if (dialogResult) {
            ...
            this.generalService.prepareHashBCY({
              ...
              Promise.all(promises).then(results => {
                ...
                console.log('payloadsignByPrincipalNew', items);
                // Gọi API 1 lần với toàn bộ danh sách đã ký
                this.generalService.SignForGVCN({items: items}).subscribe(res => {
                  this.isShowSignLoadingPrincipal = false;
                  if (res.code === 0) {
                    this.loadGrid();
                    this.notificationService.showNotification(Constant.SUCCESS, 'Ký thành công tất cả học bạ');
  ...
  requestSigned() {
    ...
    result.then((dialogResult) => {
      if (dialogResult) {
        ...
        this.generalService.signMultipleByGVCNVer2(payload, this.isElementary).subscribe(en => {
          this.isShowSignLoading = false;
          if (en.code === 0) {
  ...
  checkDisableButtonSign(){
    ...
    if (this.status40Count === 0 && this.status50Count === 0 && this.status20Count ===0) return true;
    return false;
  }
}
