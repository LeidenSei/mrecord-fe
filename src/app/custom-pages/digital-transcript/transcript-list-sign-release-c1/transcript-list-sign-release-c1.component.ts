...
@Component({
  selector: 'app-transcript-list-sign-release-c1',
  templateUrl: './transcript-list-sign-release-c1.component.html',
  styleUrls: ['./transcript-list-sign-release-c1.component.scss']
})
export class TranscriptListSignReleaseC1Component implements OnInit {
  isShowEdit = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
  ...
  async ngOnInit() {
    ...
    forkJoin([
      ...
      this.generalService.getListClassGDByGVCN(user.data.schoolId, this.filterSchoolYear, user.data.personId),
  ...
  async loadGrid() {
    this.generalService.getListHocBaNew(this.schoolId, this.filterClassId, this.filterSchoolYear).subscribe(res => {
      this.datas = res;
      let index = 1;
  ...
  schoolYearChange($event) {
    ...
    this.generalService.getListClassGDByGVCN(this.schoolId, this.filterSchoolYear, this.user.data.personId).subscribe(res => {
      this.classSource = res;

      if (this.classSource.length === 1) {
        this.gradeSource = [this.classSource[0].grade];
        this.filterGrade = this.gradeSource[0];
      }
      if (this.filterGrade) {
        this.filterClassSource = this.classSource.filter(en => en.grade === this.filterGrade);
  ...
  handleSelectionChange($event) {
    ...
    this.packagedCount = $event.selectedRowKeys.filter(en => en.status == 50 && !en.isSyncSGD).length;
    ...
    vgca_sign_msg(sender, json_prms, (sender: any, res: any) => {
      ...
      if (rv && rv.Status == 0) {
        if (type === 'CBQL') {
          ...
          console.log('decodeBase64CBQL', decodeBase64);
          if (!decodeBase64.includes('MST:')) {
            this.certCQBL_Base64 = rv.CertBase64;
          } else {
            let result = confirm(`Bạn có chắc đã lấy đúng chứng thư số của CQBL không`, 'Xác nhận');
            result.then((dialogResult) => {
              if (dialogResult) {
                this.certCQBL_Base64 = rv.CertBase64;
              } else {
      ...
      if (!isAvailable) {
        ...
        result.then((dialogResult) => {
          if (dialogResult) {
            ...
            const sender = document.getElementById("btn-sign");
            if (!this.certCQBL_Base64) {
              this.notificationService.showNotification(Constant.ERROR, 'Không tồn tại file ký số của GVCN. Vui lòng ký lại bước GVCN');
              return;
            }
  ...
  doPackaged() {
    let result = confirm(`Bạn có chắc muốn hoàn thành dữ liệu chuẩn bị nộp sở không?`, 'Xác nhận');
    result.then((dialogResult) => {
      if (dialogResult) {
        //this.isShowSignLoadingPrincipal = true;
        //this.isShowConfirmPackaged = true;
        this.callAPIPackaged();
      }
    });
  }
  callAPIPackaged(){
    this.isShowConfirmPackaged = false;
    this.isShowSignLoadingPrincipal = true;
    this.generalService.packagedHocBa({hocbaIds: this.selectedItems}).subscribe(en => {
      this.isShowSignLoadingPrincipal = false;
      if (en.code === 0) {
        this.notificationService.showNotification(Constant.SUCCESS, 'Tạo gói tin thành công');
        this.loadGrid();
      } else {
        this.notificationService.showNotification(Constant.ERROR, 'Tạo gói tin không thành công.');
      }
    }, error => {
