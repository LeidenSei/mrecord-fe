...
@Component({
  selector: 'app-principal-subject-approval-v2',
  templateUrl: './principal-subject-approval-v2.component.html',
  styleUrls: ['./principal-subject-approval-v2.component.scss']
})
export class PrincipalSubjectApprovalV2Component implements OnInit {
  isShowEdit = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
  ...
  async loadGrid() {
    this.generalService.getLopGDBySubjectTeacher(this.schoolId, this.filterTeacherId, this.filterSchoolYear, this.filterSubjectId, this.filterClassIds).subscribe(res => {
      this.datas = res;
      let index = 1;
  ...
  teacherChange($event) {
    ...
    this.teacherAssignSource = $event.itemData.phaN_CONG_GIANG_DAY;
  ...
  selectSubject(){
    ...
    this.filterClassIds = result.map(en => en.id);
