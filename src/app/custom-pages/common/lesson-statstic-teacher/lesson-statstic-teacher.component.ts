...
@Component({
  selector: 'app-lesson-statstic-teacher',
  templateUrl: './lesson-statstic-teacher.component.html',
  styleUrls: ['./lesson-statstic-teacher.component.scss']
})
export class LessonStatsticTeacherComponent implements OnInit {
  isShowConfig = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
    ...
    forkJoin([
      ...
      columns.push({
        ...
        calculateCellValue: (data) => {
          ...
          return this.dynamicColumns.filter(en => en !== 'soKhoaHocMonHoc_ChiaSeHocLieuSoChung' && en != 'soKhoaHocMonHoc_ChiaSeHocLieuSoPhongGD').reduce((sum, col) => sum + (data[col] || 0), 0);
        },
        summaryType: 'sum'  // Tính tổng theo dòng
      });
      this.dynamicColumns.push('soKhoaHocMonHoc_ChiaSeHocLieuSoChung'); // Lưu lại danh sách các cột động
      columns.push({
        fixedPosition: 'right',
        width: 130,
        caption: `Chia sẻ kho chung`,
        dataField: 'soKhoaHocMonHoc_ChiaSeHocLieuSoChung',
        fixed: true
      });
      /*if (this.user.data.schoolId === '66f94b118491e60eec1394fb') {

      }*/
      this.dynamicColumns.push('soKhoaHocMonHoc_ChiaSeHocLieuSoPhongGD'); // Lưu lại danh sách các cột động
      columns.push({
        fixedPosition: 'right',
        width: 130,
        caption: `Chia sẻ kho PGD`,
        dataField: 'soKhoaHocMonHoc_ChiaSeHocLieuSoPhongGD',
        fixed: true
      });
      columns.push({
        fixedPosition: 'right',
  ...
  async loadGrid() {
    ...
    this.generalService.getThongKeBaiGiang(payload).subscribe(res => {
      this.datas = res;
      let index = 1;
