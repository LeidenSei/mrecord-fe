...
@Component({
  ...
  templateUrl: './lesson-list-teacher.component.html',
  styleUrls: ['./lesson-list-teacher.component.scss']
})
export class LessonListTeacherComponent implements OnInit {
  isShowConfig = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
  ...
  async ngOnInit() {
    ...
    for (let i = 1; i <= 7; i++) {
      this.monthSource.push({id: `${i}-${this.user.data.currentYear + 1}`, name: `Tháng 0${i}/${this.user.data.currentYear + 1}`});
    }
    setTimeout(()=>{
      this.filterMonth = `${today.getMonth() + 1}-${today.getMonth() <= 7 ? this.user.data.currentYear + 1 : this.user.data.currentYear}`;
    }, 0);
  ...
  async loadGrid() {
    ...
    let year = 0;
    if (this.filterMonth){
      month = this.filterMonth.split('-')[0];
      year = this.filterMonth.split('-')[1];
    }
    let payload = {
      "month": month,
      "year": year,
    ...
    this.generalService.getThongKeBaiGiangGiaoVienThang(payload).subscribe(res => {
      this.datas = res;
      let index = 1;
      this.datas.forEach(en => {
        en.stt = index++;
        en.ttcmDuyet = en.ttcmDuyet ? '✓' : '';
        en.bghDuyet = en.bghDuyet ? '✓' : ''
      });
  ...
  onExporting(e: ExportingEvent) {
    ...
    const fileName = `DANH_SACH_BAI_GIANG_${this.filterMonth}_DEN_NGAY_${toDay.getDay() + 1}_${toDay.getMonth() + 1}.xlsx`;
  ...
  monthChange($event) {
    this.filterMonth = $event.itemData.id;
    this.loadGrid();
  }
}
