...
@Component({
  selector: 'app-message-statistic-teacher',
  templateUrl: './message-statistic-teacher.component.html',
  styleUrls: ['./message-statistic-teacher.component.scss']
})
export class MessageStatisticTeacherComponent implements OnInit {
  isShowConfig = false;
  datas = [];
  dataItem: any;
  taiLieuItem: any;
  ...
  statusSource = [
    {id: 0, name: 'Tất cả'},
    {id: 10, name: 'Tổ trưởng chuyên môn duyệt'},
    {id: 20, name: 'Ban giám hiệu duyệt'},
  ];
  exportTexts = {
  ...
  async ngOnInit() {
    ...
    forkJoin([
      ...
      const columns: DataGridColumn[] = [
        ...
        {dataField: 'allCount', caption: 'Tổng số tin', width: 140, fixed: false},
        {dataField: 'ottSuccessCount', caption: 'Tin gửi App', width: 140, fixed: false},
        /*{dataField: 'smsCount', caption: 'Tổng số tin', width: 200, fixed: false},*/
        {dataField: 'smsSuccessCount', caption: 'SMS thành công', width: 140, fixed: false},
        {dataField: 'failureCount', caption: 'SMS thất bại', width: 140, fixed: false},
        {dataField: 'absenceCount', caption: 'Tin điểm danh', width: 140, fixed: false},
        {dataField: 'reviewCount', caption: 'Tin nhận xét', width: 140, fixed: false},
        {dataField: 'otherCount', caption: 'Tin thông báo', width: 140, fixed: false},
        {dataField: 'mediaCount', caption: 'Tin đa phương tiện', width: 140, fixed: false},
      ];
      this.dynamicColumns = [];
  ...
  async loadGrid() {
    let payload = {
      ...
      approveStatus: this.filterStatus
    }
    if (this.filterStatus === 0) {
      delete payload.approveStatus;
    }
    console.log(payload);
    ...
    this.generalService.getThongKeTinNhan(payload).subscribe(res => {
      this.datas = res;
      let index = 1;
  ...
  openPopupDetail(data) {
    ...
    this.viewResultTitle = `Danh sách tin nhắn của giáo viên ${data.hoTen} thời gian từ ${formattedFromDate} - ${formattedToDate}`;
    this.detailItem = {
      fromDate: this.fromDate,
      toDate: this.toDate,
      schoolId: this.user.data.schoolId,
    ...
    if (this.filterStatus === 0) {
      delete this.detailItem.approveStatus;
    }
  ...
  onExporting(e: ExportingEvent) {
    ...
    const fileName = `THONG_KE_TIN_NHAN_tu_${formattedFromDate}_den_${formattedToDate}.xlsx`;
